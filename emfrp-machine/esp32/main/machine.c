...
static inline value_t *alloc_obj(int len)
{
#ifdef EMFRP_DEBUG_OBJCNT
    ++obj_cnt;
#endif
...
{
    emfrp_result_t res;
    em->v_stack = malloc(sizeof(value_t) * STACK_SIZE);
...
emfrp_result_t emfrp_exec(emfrp_machine_t *em, uint8_t *ip)
{
    ...
    while (1)
    {
        ...
        case BC_ALLOC_OBJ_1:
            ...
            break;
        case BC_ALLOC_OBJ_2:
            tmp_v.obj = malloc(3 * sizeof(value_t));
        ...
        case BC_ALLOC_OBJ_U8:
            tmp_v.obj = alloc_obj((int)READ_U8() + 1);
            READ_U32(tmp_int);
            tmp_v.obj->obj_header = (uint32_t)tmp_int;
            tmp_byte = OBJ_ENTRY_NUM(tmp_v);
            for (int i = 0; i < tmp_byte; ++i)
